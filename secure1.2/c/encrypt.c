...
int main(int argc,char *argv[])   //文件名作为命令行参数
{
	FILE *fp;
	int i,num;
	char buffer[1024*1024] = {0};  //定义缓冲区并初始化为0
	...
	//(为了替换原始文件内容，我在这以只读方式打开文件，把文件内容读到缓冲区中，关闭文件)
	//(再重新以只写方式打开文件，对缓冲区中的内容进行加5加密，并把新内容写入到文件中)
	
	fp = fopen(argv[1],"w");              //重新以只写方式打开文件
	for(i=0;i<num-1;i++){
		buffer[i] = buffer[i] + 5;        //对缓冲去内容的每一个字符进行"+5"的加密操作
	}
